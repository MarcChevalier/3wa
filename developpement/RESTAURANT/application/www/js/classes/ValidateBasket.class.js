...
ValidateBasket.prototype.load = function() {
    ...
    for (var i = 0; i < this.items.length; i++) {
		total += (parseInt(this.items[i].quantity)*parseFloat(this.items[i].price)) ;
		var tr = $('<tr>');
        tr.append('<td><em>'+this.items[i].name+'</em></td><td class="number">'+this.items[i].quantity+'</td><td class="number">'+this.items[i].price+'</td><td class="number">'+(parseInt(this.items[i].quantity)*parseFloat(this.items[i].price)).toFixed(2)+'</td>');
		tbody.append(tr);
	}
